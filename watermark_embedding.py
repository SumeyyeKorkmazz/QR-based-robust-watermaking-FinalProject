...
def watermark_embedding(cover_path, watermark_path, alpha, beta):
    ...
    # OpenCV YCrCb: Y=0, Cr=1, Cb=2. Note that MATLAB uses YCbCr (Y, Cb, Cr).
    # However, the important part is 'Y'. In OpenCV YCrCb, Y is channel 0.
    cover_ycbcr = cv2.cvtColor(cover, cv2.COLOR_RGB2YCrCb)
    
    Y = cover_ycbcr[:, :, 0].astype(np.float32)
    
    # 3. Apply DWT (Level 1 Haar)
    # pywt returns LL, (LH, HL, HH)
    ...
    # 8. Reconstruct Image
    # idwt2 requires coeffs: LL, (LH, HL, HH)
